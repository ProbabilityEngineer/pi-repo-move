...
`pi-repo-move` should participate in the same superseded/deletion-candidate model as `pi-session-move`: source bucket JSONLs copied by `/repo-move` should remain physically preserved, but canonical store marks should indicate they are superseded and manual-review deletion candidates.

## Design

After appending each repo_move manifest record, upsert minimal canonical store rows and observation_marks equivalent to pi-session-move semantics. Use `tool: pi-repo-move`, source `source_pi_repo_move_manifest`, edge metadata with operationType/sourceRepo/targetRepo, and mark source observations as `superseded` and `deletion_candidate` with manual_review_required=1. Do not delete or stage source files.
